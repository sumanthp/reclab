...
- Operational basics for running this beyond a single quick local try: upload size limits, a concurrency cap on training jobs (extra requests queue instead of piling up threads), job-store retention pruning, and a `docker-compose` health-check gate — see [Configuration](#configuration) below. A frontend test suite (38 Vitest + React Testing Library tests) now sits alongside the backend's 96 pytest tests, both run in CI.
